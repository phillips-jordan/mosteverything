...
// Preload game images
var images = {};
['enemy.png', 'stars.png', 'player.png', 'BGTest.jpg', 'SpriteSheet.png', 'explosion1.png', 'explosion2.png',
    'explosion3.png', 'explosion4.png', 'explosion5.png', 'background1.jpg','run4.png','run5.png','jump1.png',
    'jump2.png', 'jump3.png', 'over1.png', 'over2.png', 'over3.png', 'over4.png', 'over5.png',
    'run3.png', 'back.png', 'shoot1.png', 'shoot2.png', 'projectile1.png', 'projectile2.png',
    'projectile3.png', 'projectile4.png','cloud1.png','cloud2.png', 'enemy1.png','enemy2.png',
    'enemy31.png','enemy32.png','death1.png','death2.png','death3.png'].forEach(imgName => {
...
class Projectile {
    ...
    update(timeDiff) {
        ...
        if (this.speed > 0){
            this.sprite=images['projectile'+Math.round(Math.random()*(4-1)+1)+'.png']
        }
    }
    render(ctx) {
        ctx.drawImage(this.sprite, this.x, this.y);
...
class Enemy3 extends Entity {
    ...
    update(timeDiff) {
        ...
        this.sprite=images['enemy3'+Math.round(Math.random()*(2-1)+1)+'.png']
        if (this.x > playerX+PLAYER_WIDTH/2) {
            this.x = this.x - timeDiff * this.speed / 2;
        }
        if (this.y+ENEMY3_HEIGHT <= playerY+70 && this.y + ENEMY3_HEIGHT < GAME_HEIGHT - 10) {
        ...
        if (this.x < playerX-PLAYER_WIDTH/2) { this.x = this.x + timeDiff * this.speed / 2; }
    }
...
class Player extends Entity{
    ...
    update(timeDiff) {
        if(directions.left==true && this.x > 0 && this.x < GAME_WIDTH - PLAYER_WIDTH){
        this.x = this.x - timeDiff * this.speed;
        playerX = this.x
        ...
        if (directions.right == true && this.x > 0 && this.x < GAME_WIDTH - PLAYER_WIDTH){
        this.x =this.x + timeDiff *this.speed;
            playerX = this.x
            if(jumping==false&&overHead==false){
        ...
        if(directions.up == true){
            ...
            setTimeout(()=>{this.sprite=images['player.png']},400   )
        }
        }
...
class Cloudback extends Entity{
    constructor(x){
        ...
        this.sprite= images['cloud'+Math.round(Math.random()*(2-1)+1)+'.png']
        this.speed= Math.random()/6 + 0.2
    }
    update(timeDiff) {
        this.x = this.x - timeDiff * this.speed;
...
class Engine {
    constructor(element) {
        ...
        canvas.setAttribute('style', "position: absolute;  left: 50%;margin-left:-750px; top: 50%;margin-top:-300px; border:2px solid blue");
    ...
    setUpCf() {
        ...
        this.enemies[enemySpot] = new Enemy(enemySpot * ENEMY_HEIGHT - 2*ENEMY_HEIGHT)
    }
    addEnemy2() {
        var enemySpots = 10;
        var enemySpot;
    ...
    projectileFire(){
        ...
        overHead(){
            if(overHead==true){
                ...
                if(this.charge>0){this.charge-=80;
                    ...
                    setTimeout(()=>{if(overHead==true){this.player.sprite= images['over5.png']}}, 100)
                    this.enemies.forEach((ene, i) => {
                        if (ene.x + ENEMY_WIDTH > this.player.x+40 && ene.x < this.player.x + (PLAYER_WIDTH * 0.8) && ene.y + ENEMY_HEIGHT > (GAME_HEIGHT - 1.3*PLAYER_HEIGHT) && ene.y + ENEMY_HEIGHT < GAME_HEIGHT + (PLAYER_HEIGHT * 0.8)) {
                            ene.life-=3
                            if (ene.life<=0){
                            if(ene.id==3){
                                enemy3--
    ...
    start() {
        ...
        document.addEventListener('keydown', e => {
            ...
            if (e.keyCode === 81) {
                ...
                setTimeout(()=>{this.player.sprite=images['shoot2.png'], 50})
                this.projectileFire();
                }
            }
            if (e.keyCode === 32){
                ...
                setTimeout(()=>{this.player.sprite=images['jump2.png']}, 110)
                setTimeout(()=>{this.player.sprite=images['jump3.png']}, 160)
                setTimeout(()=>{directions.up=false;directions.down=true;setTimeout(()=>{
                    }, 300)
                }, 300)
            }}
            if (e.keyCode === 87){
        ...
        document.addEventListener('keyup', e => {
            ...
            if (e.keyCode === 81){
                 setTimeout(()=>{shooting=false;
                this.player.sprite=images['player.png']},60)
             }
        });
    ...
    gameLoop() {
        ...
        this.enemies.forEach((enemy, enemyIdx) => {
            ...
            if (enemy.y > GAME_HEIGHT-ENEMY_HEIGHT || enemy.x+ENEMY_WIDTH < 0) {
                ...
                setTimeout(() => { { enemy.sprite = images['explosion2.png'] } }, 40)
                setTimeout(() => { { enemy.sprite = images['explosion3.png'] } }, 80)
                setTimeout(() => { { enemy.sprite = images['explosion4.png'] } }, 120)
                setTimeout(() => { { enemy.sprite = images['explosion5.png'] } }, 160)
                setTimeout(()=>{
        ...
        this.enemies.forEach((ene, i)=>{
            console.log(this.enemies)
              if(ene.id==1){
                  //console.log('enemyX', ene.x,'enemY',ene.y,'playX',this.player.x,'playY',this.player.y)
                  if (ene.x + (ENEMY_WIDTH * 0.4) < this.player.x + PLAYER_WIDTH && ene.x + (ENEMY_WIDTH) > this.player.x && ene.y + (ENEMY_HEIGHT * 0.8) > this.player.y + (PLAYER_HEIGHT*0.8) && ene.y < this.player.y + PLAYER_HEIGHT*0.8)   
                   {
                       ...
                       {playerLife--}}}
                        ...
                        if (ene.x + (ENEMY2_WIDTH * 0.2) < this.player.x + PLAYER_WIDTH && ene.x + (ENEMY2_WIDTH) > this.player.x && ene.y + (ENEMY2_HEIGHT * 0.8) > this.player.y + (PLAYER_HEIGHT * 0.8) && ene.y < this.player.y + PLAYER_HEIGHT * 0.8)   
                        {
                       ...
                       if (ene.x + (ENEMY3_WIDTH * 0.5) < this.player.x + PLAYER_WIDTH && ene.x + (ENEMY3_WIDTH) > this.player.x && ene.y + (ENEMY3_HEIGHT) < this.player.y + (PLAYER_HEIGHT) && ene.y+ENEMY3_HEIGHT > this.player.y)   
                        {
                        enemy3--
                        console.log(enemy3, ene)
                        delete this.enemies[i];
                        if (overHead == false) { playerLife-- }
                   }}
                    })
        this.playerProj.forEach((projectile, j) => {
                this.enemies.forEach((enemy, i) => {
                    if(enemy.id==1){
                    if (enemy.x+(ENEMY_WIDTH*0.2) < projectile.x+PROJECTILE_WIDTH && enemy.x+(ENEMY_WIDTH) > projectile.x && enemy.y + (ENEMY_HEIGHT*0.8) > projectile.y+(PROJECTILE_HEIGHT/2)&&enemy.y<projectile.y+PROJECTILE_HEIGHT/2) {
                    
                        this.playerProj[j].speed = 0
                    this.playerProj[j].sprite = images['explosion1.png']
                    setTimeout(() => { {projectile.sprite = images['explosion2.png']}}, 40)
                    setTimeout(() => { {projectile.sprite = images['explosion3.png']}}, 80)
                    setTimeout(() => { {projectile.sprite = images['explosion4.png']}}, 120)
                    setTimeout(() => { {projectile.sprite = images['explosion5.png']}}, 160)
                    this.score+=100
                    enemy.life--
                    setTimeout(() =>{if(projectile){delete this.playerProj[j];
                       }} , 200)
                    if (enemy.life==0){
                        if(enemy.id==3){
                            enemy3--
                    ...
                    if(enemy.id==2){
                    if (enemy.x + (ENEMY2_WIDTH * 0.2) < projectile.x + PROJECTILE_WIDTH && enemy.x + (ENEMY2_WIDTH) > projectile.x && enemy.y + (ENEMY2_HEIGHT * 0.8) > projectile.y + (PROJECTILE_HEIGHT / 2) && enemy.y < projectile.y + PROJECTILE_HEIGHT / 2) {
                    ...
                    if(enemy.id==3){
                        if(enemy.x + (ENEMY3_WIDTH * 0.5) < projectile.x + PROJECTILE_WIDTH && enemy.x + (ENEMY3_WIDTH) > projectile.x && enemy.y + (ENEMY3_HEIGHT) < projectile.y + (PROJECTILE_HEIGHT) && enemy.y + ENEMY3_HEIGHT > projectile.y) {
        ...
        if (this.isPlayerDead()) {
            ...
            this.player.render(this.ctx); // draw the player
            this.playerProj.forEach(projectile => projectile.render(this.ctx))
            setTimeout(()=>{this.player.sprite=images['death2.png'];
            this.bg.forEach(bgx => bgx.render(this.ctx))
            this.cb.forEach(c => c.render(this.ctx))
            this.enemies.forEach(enemy => enemy.render(this.ctx)); // draw the enemies
            this.cf.forEach(c => c.render(this.ctx))
        ...
        setTimeout(() => { this.player.sprite = images['death3.png'];
        this.bg.forEach(bgx => bgx.render(this.ctx))
        this.cb.forEach(c => c.render(this.ctx))
        this.enemies.forEach(enemy => enemy.render(this.ctx)); // draw the enemies
        this.cf.forEach(c => c.render(this.ctx))
